...
├── .github/workflows/  CI, keepalive, backup workflows
...
- Celery jobs must be durable through `job_runs`; do not rely on Redis result backend or local ephemeral files as the source of truth.
...
- Use Neon's direct database URL for migrations and pooled URL for the app, matching the handbook.
...
- Automatic rollover copies structure and identity links, never grades, audit rows, or school
  numbers; grade 8 students are not promoted.

Before suggesting a schema change, answer:

1. Which handbook step requires this table/column?
2. Is this already represented in the 14-table design?
...
- Use machine codes in API errors; frontend translates them.
...
pnpm --filter @flrc/teacher test
pnpm --filter @flrc/admin test
